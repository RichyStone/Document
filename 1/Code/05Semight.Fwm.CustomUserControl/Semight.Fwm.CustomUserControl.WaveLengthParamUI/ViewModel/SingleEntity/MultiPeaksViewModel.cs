...
{
    public partial class MultiPeaksViewModel : ObservableValidator
    {
        #region Fields

        [ObservableProperty]
        private MultiPeaksParam? bindingParam;
        ...
        private void WriteParamFuction(object recipient, MessagerTransData message)
        {
            try
            {
                var param = GlobalVariable.LCParamBulider.CurrentParam?.MultiPeaksParam;
                if (param != null)
                    HardWareManager.WriteMultiPeaksParam(param);
            }
            catch (Exception ex)
            {
                LogHelper.LogError("写入多峰参数失败", ex);
                MessageBoxHelper.ErrorBox("写入多峰参数失败");
            }
        }
        ...
        {
            BindingParam = null;
            BindingParam = GlobalVariable.LCParamBulider.CurrentParam?.MultiPeaksParam;
        }
