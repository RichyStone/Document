...
{
    public partial class CompensationViewModel : ObservableValidator
    {
        #region Fields

        [ObservableProperty]
        private DispersionCompensateParam? bindingParam;
        ...
        private void WriteParamFuction(object recipient, MessagerTransData message)
        {
            try
            {
                var param = GlobalVariable.LCParamBulider.CurrentParam?.DispersionCompensation;
                if (param != null)
                    HardWareManager.WriteCompensationParam(param);
            }
            catch (Exception ex)
            {
                LogHelper.LogError("写入色散补偿参数失败", ex);
                MessageBoxHelper.ErrorBox("写入色散补偿参数失败");
            }
        }
        ...
        {
            BindingParam = null;
            BindingParam = GlobalVariable.LCParamBulider.CurrentParam?.DispersionCompensation;
        }
