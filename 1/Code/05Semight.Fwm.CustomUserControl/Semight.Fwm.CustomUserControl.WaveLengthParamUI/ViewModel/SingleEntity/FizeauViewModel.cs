...
{
    public partial class FizeauViewModel : ObservableValidator
    {
        #region Property

        [ObservableProperty]
        private BindingList<FizeauParam>? bindingParams;
        ...
        private void WriteParamFuction(object recipient, MessagerTransData message)
        {
            try
            {
                var param = GlobalVariable.LCParamBulider.CurrentParam?.FizeauParameters;
                if (param != null)
                    HardWareManager.WriteFizeauParam(param);
            }
            catch (Exception ex)
            {
                LogHelper.LogError("写入菲索参数失败", ex);
                MessageBoxHelper.ErrorBox("写入菲索参数失败");
            }
        }
        ...
        private void RefreshParam()
        {
            ...
            var param = GlobalVariable.LCParamBulider.CurrentParam?.FizeauParameters;
            if (param != null)
                BindingParams = new BindingList<FizeauParam>(param.OrderBy(p => p.WaveBand).ThenBy(p => p.Thickness).ToList());
        }
